...
import { emailStageRequiredSchema, type EmailStageInput } from "@/platform/lib/validation/journey";
...
// Stage 3 — Email (Resend). Instructions + 2 attestations. Resend API key
    ...
    <div className="space-y-4">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Resend handles sending transactional emails — welcome emails, password resets, payment
            receipts. The free tier gives you 100 emails per day, plenty for V1.
          </CardDescription>
          <a
        ...
        <CardContent className="space-y-6">
          ...
          <p className="text-sm text-muted-foreground">
            Same routine as before: your Resend API key goes into your local <code>.env.local</code>
            , never into this site.
          </p>
            ...
            <form onSubmit={form.handleSubmit(onContinue)} className="space-y-6" noValidate>
              <section className="space-y-2">
                ...
                <div className="rounded-md border border-amber-500/40 bg-amber-50 p-3 text-xs text-amber-900 dark:bg-amber-950/40 dark:text-amber-200">
                  <p className="font-semibold">Heads up about who Resend can email</p>
                  <p className="mt-1">
                    On the free plan, sending from <code>onboarding@resend.dev</code> only delivers
                    to <strong>the email you signed up to Resend with</strong>. Great for testing
                    yourself; not yet for reaching real customers.
                  </p>
                  <p className="mt-2 font-semibold">Now (recommended for early days):</p>
                  <p className="mt-1">
                    Email new customers yourself from your own inbox when they sign up or pay.
                    Higher-touch, better feedback, zero deliverability surprises. The code is still
                    wired up — it just won&apos;t reach anyone but you until you finish the next
                    paragraph.
                  </p>
                  <p className="mt-2 font-semibold">Later (when you&apos;re ready):</p>
                  <p className="mt-1">
                    Register a domain (Stage 5 has registrar links), verify it inside Resend (5
                    minutes), then change <code>EMAIL_FROM</code> to{" "}
                    <code>hello@yourdomain.com</code>. After that, every welcome / receipt /
                    cancellation email this app generates delivers automatically.
                  </p>
                </div>
              </section>
              ...
              <section className="space-y-2">
                <h3 className="font-semibold">
                  2. Add your two Resend values to your local <code>.env.local</code> file
                </h3>
                <p className="text-sm text-muted-foreground">
                  Open <code>.env.local</code> in your editor (the same file you&apos;ve been
                  editing all along). Paste these two lines at the bottom:
                </p>
                <CopyableCodeBlock code={ENV_TEMPLATE_RESEND} />
                <p className="text-sm text-muted-foreground">
                  Now go to your Resend dashboard and replace each placeholder with its real value:
                </p>
                <ul className="ml-5 list-disc space-y-1 text-sm text-muted-foreground">
                  <li>
                    In Resend, click <strong>API Keys</strong> in the left sidebar →{" "}
                    <strong>Create API Key</strong>. Give it any name. Copy the key — it starts with{" "}
                    <code>re_</code> — and paste it into <code>RESEND_API_KEY</code>.{" "}
                    <strong>Resend only shows this key once</strong>, so paste it into your{" "}
                    <code>.env.local</code> immediately.
                  ...
                  <li>
                    For <code>EMAIL_FROM</code>: replace <code>Your App Name</code> with your
                    app&apos;s name (the same one from Stage 1). Leave the rest as{" "}
                    <code>&lt;onboarding@resend.dev&gt;</code> for now — see the heads-up note in
                    Step 1.
                    {/* COPY_TODO: review tone — inline example. */}
                    <div className="mt-0.5 text-xs">
              ...
              <section className="space-y-2 rounded-md border border-emerald-500/30 bg-emerald-50/40 p-4 dark:bg-emerald-950/20">
                ...
                <p className="text-sm text-muted-foreground">
                  Once you&apos;ve added your Supabase, Stripe, and Resend keys, your{" "}
                  <code>.env.local</code> should have all your real values filled in. Here&apos;s an
                  example with redacted-but-realistic values — yours will have your actual keys:
                </p>
                <CopyableCodeBlock code={COMPLETED_ENV_EXAMPLE} />
                <p className="text-sm text-muted-foreground">
                  If yours looks roughly like this — same variable names, similar value shapes —
                  you&apos;re set up correctly. If anything&apos;s missing or looks very different,
                  check back through the Payments and Email stages.
                </p>
              </section>
...
function Attestations({ form }: { form: ReturnType<typeof useForm<EmailStageInput>> }) {
  return (
    <section className="space-y-3 rounded-md border bg-muted/40 p-4">
      ...
      <div className="space-y-2">
        ...
        <FormField
          ...
          render={({ field }) => (
            <AttestationBox checked={Boolean(field.value)} onChange={field.onChange}>
              I&apos;ve added my two Resend values to my local <code>.env.local</code> file
            </AttestationBox>
          )}
        />
      </div>
